...
class AmazonKindleSpider(scrapy.Spider):
    ...
    export_json = pathlib.Path(
        'exports', 'kindle_audible_ebooks-%(time)s.json')
    ...
    def selected_fields(self):
        choice = input("Field choice: manual/all: m or a")[0]
    ...
    def selected_categorical(self, field_names, field_urls):
        choice = input('Select category manual/all: m or a')[0]
        names = []
        urls = []
        if choice.lower() == 'm':
    ...
    def start_scheduler(self):
        self._start_year = int(input("start year: "))
        self._start_month = int(input("start month: "))
        self._start_date = int(input("start date: "))
        self._start_hour = int(input("start hour: "))
        self._start_minute = int(input("start minute: "))
        self._start_second = 0
        self._end_year = int(input("end year: "))
        self._end_month = int(input("end month: "))
        self._end_date = int(input("end date: "))
        self._end_hour = int(input("end hour: "))
        self._end_minute = int(input("end minute: "))
        ...
        print('Waiting for start crawling...')
        time.sleep(abs(duration_in_s))

    def stop_scheduler(self):
        then = datetime.datetime(
            self._end_year, self._end_month, self._end_date, self._end_hour, self._end_minute, self._end_second)
        now = datetime.datetime.now()
        now = now - datetime.timedelta(microseconds=now.microsecond)
        if (then == now) and (then < now):
            raise CloseSpider("Schedule time is over")

    _fields = []

    # COUNT_MAX = int(input("Enter Limit: "))
    COUNT_MAX = 10000
    ...
    def start_requests(self):
        self._fields = self.selected_fields()
    ...
    def category_parse(self, response):
        ...
        choice = input("Schedule/manual: a or m")[0]
        if choice == "a":
            # start scheduler to scrape
    ...
    def subcategory_parse(self, response):
        ...
        # select subcategory to scrape
        # subcategory_names, subcategory_urls = selected_categorical(
        #     subcategory_names, subcategory_urls, input('Select category manual/all: '))
...
if __name__ == "__main__":
    ...
    process.start()
    # process.stop() #close spider manually
